...
import { getCompaniesData, getJobsData, getTotalEmployersCount, getTotalJobsCount, getTotalUsersCount, getUsersData } from '../../../store/admin.selector';
...
export class ManagementComponent implements OnInit, AfterViewInit, OnDestroy{
  ...
  usersData$ = this._adminStore.select(getUsersData)
  employersData$ = this._adminStore.select(getCompaniesData)
  jobsData$ = this._adminStore.select(getJobsData)
  ...
  ngOnInit(): void {
      ...
      tap(currentRoute => {
        ...
        if (this.viewMode === 'users') {
          ...
          this.filterOptions = [
            ...
            { label: 'closed/ongoing', subOptions: [{ label: 'ongoing jobs', key: 'isClosed', value: 'false' }, { label: 'closed jobs', key: 'isClosed', value: 'true' }], type: 'Radio' },
    ...
    this.queryParamSubscription = this._activatedRoute.queryParamMap.pipe(
      tap(queries => {
      ...
      map(queries => {
        ...
        return this.constructQueryString(queryParams);
      }),
      tap(filterQueryString => {
        if (this.viewMode === 'users') {
    ...
    if (this.viewMode == 'users') {
      this._adminStore.select(getTotalUsersCount).pipe(
        tap((data) => this.totalEntries = data)).subscribe()
    } else if (this.viewMode == 'jobs') {
      this._adminStore.select(getTotalJobsCount).pipe(
        tap((data) => this.totalEntries = data)).subscribe()
    } else if (this.viewMode == 'companies') {
      this._adminStore.select(getTotalEmployersCount).pipe(
        tap((data) => this.totalEntries = data)).subscribe()
    }
  }
